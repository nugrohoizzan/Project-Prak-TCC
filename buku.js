document.addEventListener("DOMContentLoaded", () => {
    ...
    const getBuku = async () => {
      try {
        const response = await axios.get("http://localhost:8080/buku");
        tableBuku.innerHTML = "";
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        if (id) {
          await axios.put(`http://localhost:8080/buku/${id}`, { nama, kode_buku: kodeBuku });
        } else {
          await axios.post("http://localhost:8080/buku", { nama, kode_buku: kodeBuku });
        }
        form.reset();
    ...
    window.editBuku = async (id) => {
      try {
        const response = await axios.get(`http://localhost:8080/buku/${id}`);
    ...
    window.deleteBuku = async (id) => {
      try {
        await axios.delete(`http://localhost:8080/buku/${id}`);
        getBuku();
      } catch (error) {
